"""
============================================================================
EXPERIMENT 6.3 — LHT-ViT  Maximum Generalization & Peak Precision Framework
   EfficientNet-B0 + ViT-Small  ·  Bidirectional Co-Attention  ·  MFB
   5-Class CE (label_smoothing=0.15)  ·  Mixup+CutMix  ·  WarmRestarts
...
Fundus image  (B, 3, 224, 224)
    ...
    │
    └── HEAD : LayerNorm → 512 → GELU → Dropout(0.60) → Linear(512 → 5)
               5 raw logits  (CrossEntropyLoss, label_smoothing=0.15)

CHANGES FROM EXPERIMENT 6.2  (final thesis submission benchmark)
-----------------------------------------------------------------
1. Loss  : OrdinalSmoothMSELoss (scalar regression)
         → CrossEntropyLoss(label_smoothing=0.15)  (5-class classification)
   Returning to full 5-class CE: it provides richer gradient signal per batch
   (5 logit residuals vs. 1 scalar residual), label_smoothing=0.15 prevents
   over-confident spikes that split the train/val curves, and CE is the
   standard baseline reviewers will compare against.

2. Head  : Linear(512 → 1) + 4×sigmoid  (bounded scalar)
         → Linear(512 → 5)  (5 logits, no activation — standard CE head)

3. Mixup  + CutMix  (NEW in 6.3, p=0.5 per batch):
   • Mixup(alpha=0.2): linearly blends two images and their one-hot labels.
   • CutMix(alpha=1.0): pastes a rectangular region from a partner image;
     label mixed by area ratio.
   Applied in train_one_epoch only — val/test batches are never augmented.
   These two transforms are the single most effective technique for closing
   the train/val gap in classification networks on medical image datasets.

4. Regularisation scaled up:
   • drop_path_rate  : 0.25 → 0.40  (deeper stochastic depth in ViT-Small)
   • attn_drop_rate  : 0.10 → 0.15  (stronger attention-weight dropout)
   • head_dropout    : 0.40 → 0.60  (stronger MFB head dropout)
   • weight_decay    : 0.05         (unchanged — already strict)

5. Scheduler  : SequentialLR(LinearWarmup + Cosine)
              → CosineAnnealingWarmRestarts(T_0=10, T_mult=1, eta_min=1e-7)
   Cyclic restarts continuously disrupt late-stage training plateaux.

6. Expanded evaluation suite retained from 6.2 (all 4 CSVs + binary PNG).

ENVIRONMENT
-----------
Google Colab Pro  (L4 or A100 GPU, High-RAM runtime)
All weights, CSVs, and PNGs are saved to Google Drive at:
  /content/drive/MyDrive/DR_Experiment_6_3_Outputs/
============================================================================
"""
...
CFG = dict(
    ...
    output_dir = "/content/drive/MyDrive/DR_Experiment_6_3_Outputs",
    ...
    # ── Training ─────────────────────────────────────────────────────────────
    epochs       = 50,
    lr           = 2e-5,             # peak LR for CosineAnnealingWarmRestarts
    weight_decay = 5e-2,             # unchanged — strict L2
    eta_min      = 1e-7,
    patience     = 10,
    # CosineAnnealingWarmRestarts parameters (no warmup phase needed)
    T_0          = 10,               # restart period in epochs
    T_mult       = 1,                # period stays constant across restarts
    ...
    mfb_out        = 1024,
    num_classes    = 5,
    head_dropout   = 0.60,           # 6.3: 0.40 → 0.60
    drop_path_rate = 0.40,           # 6.3: 0.25 → 0.40
    attn_drop_rate = 0.15,           # 6.3: 0.10 → 0.15

    # ── Loss ─────────────────────────────────────────────────────────────────
    # 6.3: CrossEntropyLoss (5-class) with label_smoothing=0.15
    label_smoothing = 0.15,

    # ── Mixup / CutMix ───────────────────────────────────────────────────────
    mixup_alpha  = 0.2,
    cutmix_alpha = 1.0,
    mix_prob     = 0.5,              # probability of applying either per batch
)
...
print("=" * 72)
print("  EXPERIMENT 6.3 — LHT-ViT  Maximum Generalization & Peak Precision")
print("  5-Class CE (ls=0.15)  ·  Mixup+CutMix  ·  CosineAnnealingWarmRestarts")
print("=" * 72)
...
print(f"  Scheduler    : CosineAnnealingWarmRestarts"
      f"(T_0={CFG['T_0']}, T_mult={CFG['T_mult']}, eta_min={CFG['eta_min']})")
print(f"  Loss         : CrossEntropyLoss(label_smoothing={CFG['label_smoothing']})")
print(f"  Head         : Linear → 5 logits  (standard classification head)")
...
print(f"  Augmentation : HFlip + Rotate15 + RandomErasing(p=0.25)"
      f" + Mixup(α={CFG['mixup_alpha']}) + CutMix(α={CFG['cutmix_alpha']}, p={CFG['mix_prob']})")
print()
...
class HybridLHT_ViT(nn.Module):
    """
    Hybrid LHT-ViT with a 5-class CrossEntropyLoss head (Experiment 6.3).

    Forward pass returns raw logits of shape [B, 5].
    Mixup and CutMix mixed labels are handled entirely in the training loop —
    the model itself is label-agnostic.
    """

    def __init__(self, attn_dim=512, mfb_k=5, mfb_out=1024, num_classes=5,
                 head_dropout=0.60, drop_path_rate=0.40,
                 attn_drop_rate=0.15, pretrained=True):
        super().__init__()
        ...
        # drop_path_rate=0.40: deep stochastic depth (scaled linearly per layer).
        # attn_drop_rate=0.15: dropout applied to softmax attention weights.
        ...
        # Dropout(0.60) aggressively regularises the 1024-dim fused representation
        # before the final projection, working in concert with Mixup/CutMix.
    ...
    def forward(self, x):
        ...
        return self.head_linear(fused)                      # [B, 5]  raw logits
...
class _OrdinalSmoothMSELoss_DEPRECATED(nn.Module):  # noqa — replaced in 6.3
    """Kept for checkpoint compat only. Not used in Experiment 6.3."""
...
def _mixup_batch(images: torch.Tensor, labels: torch.Tensor,
                 alpha: float, num_classes: int = 5):
    """
    Mixup — linearly blend two images and their one-hot labels.
    Returns mixed images [B,C,H,W] and soft label tensor [B, num_classes].
    """
    lam = float(np.random.beta(alpha, alpha))
    idx = torch.randperm(images.size(0), device=images.device)
    images_mix = lam * images + (1.0 - lam) * images[idx]
    y_a = F.one_hot(labels, num_classes).float()
    y_b = F.one_hot(labels[idx], num_classes).float()
    return images_mix, lam * y_a + (1.0 - lam) * y_b


def _cutmix_batch(images: torch.Tensor, labels: torch.Tensor,
                  alpha: float, num_classes: int = 5):
    """
    CutMix — paste a random rectangle from a partner image.
    Label is mixed by area ratio (1 − λ) proportional to replaced area.
    """
    lam = float(np.random.beta(alpha, alpha))
    ...
    y_a = F.one_hot(labels, num_classes).float()
    y_b = F.one_hot(labels[idx], num_classes).float()
    return images_mix, lam_actual * y_a + (1.0 - lam_actual) * y_b


def _apply_mix(images: torch.Tensor, labels: torch.Tensor,
               mixup_alpha: float, cutmix_alpha: float,
               mix_prob: float, num_classes: int = 5):
    """
    With probability mix_prob, randomly apply either Mixup or CutMix.
    Returns (images, soft_labels_or_None, mixed: bool).
    When not mixed the caller uses hard labels with CE label_smoothing.
    """
    if np.random.rand() > mix_prob:
        return images, None, False
    if np.random.rand() < 0.5:
        imgs, soft = _mixup_batch(images, labels, mixup_alpha, num_classes)
    else:
        imgs, soft = _cutmix_batch(images, labels, cutmix_alpha, num_classes)
    return imgs, soft, True


def _ce_soft(logits: torch.Tensor, soft_labels: torch.Tensor) -> torch.Tensor:
    """Cross-entropy for soft / mixed label distributions."""
    log_probs = F.log_softmax(logits, dim=1)
    return -(soft_labels * log_probs).sum(dim=1).mean()


def train_one_epoch(model, loader, criterion, optimizer, scaler,
                    device, epoch, use_amp, cfg):
    """Training step with Mixup / CutMix batch augmentation."""
    model.train()
    running_loss = 0.0
    all_true, all_pred = [], []
    ...
    for images, labels in pbar:
        ...
        images, soft_labels, mixed = _apply_mix(
            images, labels,
        ...
        with autocast(enabled=use_amp):
            ...
            if mixed:
                loss = _ce_soft(logits, soft_labels)
            else:
...
def evaluate(model, loader, criterion, device, epoch, desc, use_amp):
    """Validation / test step — no augmentation, argmax predictions."""
    model.eval()
    running_loss = 0.0
...
def save_roc_curves(y_true, y_probs, path, n_classes=5):
    """
    One-vs-Rest ROC curves using softmax class probabilities as the ranking
    score (Experiment 6.3 — standard CE head).

    y_probs : np.ndarray of shape [N, n_classes] — softmax probabilities.
    """
    ...
    for c in range(n_classes):
        scores = y_probs[:, c] if y_probs.ndim == 2 else y_probs
...
def save_training_history(history, path):
    ...
    for ax, (tr, va, ylabel, title) in zip(axes, [
        (tr_loss, va_loss, "CrossEntropy Loss",      "Loss"),
    ...
    fig.suptitle("Experiment 6.3 — Training History  (LHT-ViT  5-Class CE)",
...
def main():
    ...
    # ── Model ─────────────────────────────────────────────────────────────────
    print("\nBuilding HybridLHT_ViT (5-class CE head) ...")
    model = HybridLHT_ViT(
    ...
    print(f"  Head output  : 5 logits  (CrossEntropyLoss, ls={CFG['label_smoothing']})")

    # ── Loss, optimizer, scheduler, AMP ──────────────────────────────────────
    # Exp 6.3: CrossEntropyLoss with label_smoothing=0.15.
    # Mixup/CutMix batches bypass this criterion and use _ce_soft() instead.
    ...
    # Exp 6.3 scheduler: CosineAnnealingWarmRestarts
    # Cyclic restarts (every T_0 epochs) prevent the model from settling into
    # sharp late-stage minima that generalise poorly to validation data.
    ...
    print(f"  Scheduler    : CosineAnnealingWarmRestarts"
          f"(T_0={CFG['T_0']}, T_mult={CFG['T_mult']})")

    scaler    = GradScaler(enabled=use_amp)
    save_path = out_dir / "best_model_LHTViT_Exp6_3.pth"
    ...
    for epoch in range(1, CFG["epochs"] + 1):
        ...
        if va_qwk > best_val_qwk:
            ...
            torch.save({
                ...
                "val_acc":  va_acc,
                "architecture": "HybridLHT_ViT [Exp 6.3 — CE head]",
                "cfg": CFG,
            }, save_path)
            saved = "*"
        else:
    ...
    log_path = out_dir / "train_log_exp6_3.json"
    ...
    print("\n  PRIMARY — 5-class DR Grading (CrossEntropyLoss, ls=0.15)")
    print("=" * 72)
    ...
    roc_path     = out_dir / "roc_curves_exp6_3.png"
    history_path = out_dir / "training_history_exp6_3.png"
    save_roc_curves(y_true, y_probs, roc_path)
    save_training_history(history, history_path)

    # ── Final summary banner ──────────────────────────────────────────────────
    print("\n" + "=" * 72)
    print("  FINAL SUMMARY  (Exp 6.3 — Maximum Generalization & Peak Precision)")
    print("=" * 72)
    print(f"  QWK                   : {te_qwk:.4f}")
    print(f"  F1-Score  (Macro)     : {te_f1_macro:.4f}")
    print(f"  F1-Score  (Micro)     : {te_f1_micro:.4f}")
